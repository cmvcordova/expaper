...
Research project scaffolding — experimentStash + Overleaf in one repo.
...
expaper/
├── expaper/
│   ├── cli.py              # Click CLI (init, tool, sync, template, link-overleaf)
...
│       └── paper/          # LaTeX templates (blank, arxiv)
...
```bash
expaper init <name> [--tools T] [--overleaf URL] [--template T] [--no-interactive] [--dry-run]
expaper tool add <name> [url]
expaper tool list [--registry]
expaper sync pull | push | status
expaper link-overleaf <url>
expaper template list | create <name> | export
```
